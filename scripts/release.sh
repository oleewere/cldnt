...
if [ "$(uname -s)" = 'Linux' ]; then
  CLDNT_SCRIPT_DIR="`dirname "$(readlink -f "$0")"`"
else
  CLDNT_SCRIPT_DIR="`dirname "$(readlinkf "$0")"`"
fi

CLDNT_ROOT_DIR="`dirname \"$CLDNT_SCRIPT_DIR\"`"
...
function update_readme_version() {
  local next_release="$1"
  local readme_md_location="$CLDNT_ROOT_DIR/README.md"
  local version_number=$(get_version_number $next_release)
  sed -i.bak "s/CLDNT_VERSION=[[:digit:]]\.[[:digit:]]\.[[:digit:]]/CLDNT_VERSION=${version_number}/" "$readme_md_location"
...
function run_release() {
  docker run -w /go/src/github.com/oleewere/cldnt -e GITHUB_TOKEN=$GITHUB_TOKEN --rm -v $CLDNT_ROOT_DIR/vendor/:/go/src/ -v $CLDNT_ROOT_DIR:/go/src/github.com/oleewere/cldnt goreleaser/goreleaser:latest --debug --rm-dist
}

function build_only() {
  docker run -w /go/src/github.com/oleewere/cldnt -e GITHUB_TOKEN=$GITHUB_TOKEN --rm -v $CLDNT_ROOT_DIR/vendor/:/go/src/ -v $CLDNT_ROOT_DIR:/go/src/github.com/oleewere/cldnt goreleaser/goreleaser:latest --snapshot --debug --rm-dist --skip-publish
}
